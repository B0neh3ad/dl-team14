...
load_model_path = os.path.join('finetuned_models', base_model)
save_model_path = os.path.join('finetuned_models_ttt', base_model)
inference_cache = os.path.join(output_path, 'inference_cache')
submission_file = os.path.join(output_path, 'submission.json')

# load evaluation dataset
arc_eval_set = ArcDataset.load_from_json_dl('../../dataset', size=3, seed=42, shuffle=False, eval=True)

# load model
retrain = True
model, tokenizer = load_unsloth_4bit(load_model_path)
