...
def solve_f25ffba3(I):
    ...
    for i in range(h):
        for j in range(w):
            c = I[i][j]
            if c != bg:
                jm = w - 1 - j   # horizontal mirror column
                O[i][jm] = c
    # Convert back to tuple grid expected by the framework
    return O
...
def solve_0520fde7(I):
    ...
    # ── 1. Locate a horizontal divider row (if any) ────────────────────────
    h_div = next(
        (r for r, row in enumerate(I)
         if row[0] != 0 and len(set(row)) == 1 and r == H - r - 1),
        None,
    )

    # ── 2. Locate a vertical divider column (if no horizontal one) ─────────
    v_div = None
    if h_div is None:
        for c in range(W):
            col = [I[r][c] for r in range(H)]
            if col[0] != 0 and len(set(col)) == 1 and c == W - c - 1:
                v_div = c
                break

    # ── 3. Slice the halves and compute the intersection ───────────────────
    if h_div is not None:  # horizontal split
        top, bottom = I[:h_div], I[h_div + 1:]
        return [
            [2 if top[r][c] and bottom[r][c] else 0 for c in range(W)]
            for r in range(h_div)
        ]

    if v_div is not None:  # vertical split
        left = [row[:v_div] for row in I]
        right = [row[v_div + 1:] for row in I]
        return [
            [2 if left[r][c] and right[r][c] else 0 for c in range(v_div)]
            for r in range(H)
        ]

    # No valid divider found — return empty grid (should not occur in valid tasks)
    return []
...
def solve_00d62c1b(I):
    ...
    # Track dominant‐colour cells that are connected to the border.
    seen = [[False] * w for _ in range(h)]
    from collections import deque
    q = deque()

    # Seed the flood‑fill with border cells of the dominant colour.
    for i in range(h):
        for j in range(w):
            if (i == 0 or i == h - 1 or j == 0 or j == w - 1) and I[i][j] == dom:
                seen[i][j] = True
                q.append((i, j))

    # Standard BFS flood‑fill over orthogonal neighbours.
    while q:
        i, j = q.popleft()
        for di, dj in ((-1, 0), (1, 0), (0, -1), (0, 1)):
            ni, nj = i + di, j + dj
            if 0 <= ni < h and 0 <= nj < w and not seen[ni][nj] and I[ni][nj] == dom:
                seen[ni][nj] = True
                q.append((ni, nj))

    # Construct the output grid, recolouring interior dominant cells to FOUR.
    O = [list(row) for row in I]
    for i in range(h):
        for j in range(w):
            if I[i][j] == dom and not seen[i][j]:
                O[i][j] = FOUR
...
def solve_007bbfb7(I):
    ...
    # 빈 출력 격자 만들기
    O = [[0] * (W * W) for _ in range(H * H)]

    # 트리거 셀마다 원본 패턴 복사
    for r in range(H):
        for c in range(W):
            if I_list[r][c] != 0:          # 0 이 아닌 셀만 사용
                base_r, base_c = r * H, c * W
                for dr in range(H):
                    for dc in range(W):
                        O[base_r + dr][base_c + dc] = I_list[dr][dc]
...
def solve_496994bd(I):
    ...
    for i in range(h):
        for j in range(w // 2):
            ...
            if c1 == bg and c2 != bg:
                O[i][j] = c2
            elif c2 == bg and c1 != bg:
                O[i][jm] = c1
            # If both are background or both non‑background we leave them as is.
    return O
...
def solve_05f2a901(I):
    ...
    # 색상 2와 8 객체 먼저 시도 (원래 패턴)
    x2 = colorfilter(x1, TWO)
    x4 = colorfilter(x1, EIGHT)
    
    if size(x2) > 0 and size(x4) > 0:
        x3 = first(x2)
        x5 = first(x4)
        x6 = gravitate(x3, x5)
        O = move(I, x3, x6)
    ...
    non_bg_objects = sfilter(x1, lambda obj: color(obj) != bg_color)
    
    if size(non_bg_objects) < TWO:
        return I
    
    # 크기별로 분류
    single_pixels = sfilter(non_bg_objects, lambda obj: size(obj) == ONE)
    multi_cell_objects = sfilter(non_bg_objects, lambda obj: size(obj) > ONE)
    
    # Case 1: 단일 픽셀이 있는 경우 - 단일 픽셀을 앵커로 사용
    if size(single_pixels) > 0 and size(multi_cell_objects) > 0:
        # 가장 큰 다중 셀 객체를 단일 픽셀 쪽으로 이동
        anchor = first(single_pixels)
        moving_obj = argmax(multi_cell_objects, size)
        direction = gravitate(moving_obj, anchor)
        O = move(I, moving_obj, direction)
        return O
    
    # Case 2: 다중 셀 객체만 있는 경우
    if size(multi_cell_objects) >= TWO:
        # 가장 작은 객체를 가장 큰 객체 쪽으로 이동
        smallest = argmin(multi_cell_objects, size)
        largest = argmax(multi_cell_objects, size)
        
        if smallest != largest:
            direction = gravitate(smallest, largest)
            O = move(I, smallest, direction)
            return O
    
    # Case 3: 단일 픽셀들만 있는 경우
    if size(single_pixels) >= TWO:
        # 색상이 다른 두 단일 픽셀 찾기
        colors = apply(color, single_pixels)
        unique_colors = dedupe(totuple(colors))
        
        if size(unique_colors) >= TWO:
            color1 = first(unique_colors)
            color2 = first(remove(color1, unique_colors))
            
            pixel1_objs = colorfilter(single_pixels, color1)
            pixel2_objs = colorfilter(single_pixels, color2)
            
            if size(pixel1_objs) > 0 and size(pixel2_objs) > 0:
                pixel1 = first(pixel1_objs)
                pixel2 = first(pixel2_objs)
                direction = gravitate(pixel1, pixel2)
                O = move(I, pixel1, direction)
                return O
    
    # Case 4: 마지막 대안 - 색상별로 처리
    colors = apply(color, non_bg_objects)
    unique_colors = dedupe(totuple(colors))
    ...
    if size(unique_colors) >= TWO:
        ...
        objs1 = colorfilter(non_bg_objects, color1)
        objs2 = colorfilter(non_bg_objects, color2)
        
        if size(objs1) > 0 and size(objs2) > 0:
            obj1 = argmin(objs1, size)  # 첫 번째 색상의 가장 작은 객체
            obj2 = argmin(objs2, size)  # 두 번째 색상의 가장 작은 객체
            
            # 더 작은 객체를 더 큰 객체 쪽으로 이동
            if size(obj1) <= size(obj2):
                direction = gravitate(obj1, obj2)
                O = move(I, obj1, direction)
            else:
                direction = gravitate(obj2, obj1)
...
def solve_017c7c7b(I):
    ...
    # 1. Recolour
    rows = [[2 if c else 0 for c in row] for row in I]

    # 2. Find shortest period p
    tuple_rows = [tuple(r) for r in rows]
    p = h
    for cand in range(1, h):
        if all(tuple_rows[i] == tuple_rows[i + cand] for i in range(h - cand)):
            p = cand
            break

    # 3. Build 9‑row output by repeating the pattern
    return [list(tuple_rows[i % p]) for i in range(9)]
...
def solve_a1570a43(I):
    ...
    # ───────────────────────────────── 바운딩박스 잘라내기
    sub = [[I[r][c] for c in range(min_c, max_c + 1)]
           for r in range(min_r, max_r + 1)]

    # ───────────────────────────────── 90° 시계회전
    # 회전 결과 크기는 (box_w × box_h)
    sub_rot = [[sub[box_h - 1 - r][c] for r in range(box_h)]
               for c in range(box_w)]
    rot_h, rot_w = len(sub_rot), len(sub_rot[0])

    # ───────────────────────────────── 삽입 위치 계산
    ins_r = min_r                       # 세로 위치는 그대로
    ins_c = min_c
    if ins_c + rot_w > W:               # 오른쪽 경계 넘침 보정
        ins_c = W - rot_w
    ...
    # 2) 회전된 블록 덮어쓰기
    for dr in range(rot_h):
        for dc in range(rot_w):
            out[ins_r + dr][ins_c + dc] = sub_rot[dr][dc]
...
def solve_05269061(I):
    ...
    # ------------------------------------------------------------------
    # 1.  Collect every non‑zero clue (row, col, colour)
    # ------------------------------------------------------------------
    clues = [(r, c, I[r][c])
             for r in range(h) for c in range(w) if I[r][c] != 0]
    if not clues:                       # edge‑case: blank grid → just return it
        return I

    # Frequency table for tie‑breaking + filling
    freq = {}
    for _, _, v in clues:
        freq[v] = freq.get(v, 0) + 1
    colours_by_freq = sorted(freq, key=freq.get, reverse=True)

    # ------------------------------------------------------------------
    # 2.  Search across orientations and periods
    # ------------------------------------------------------------------
    # Orientation functions – they all return *index* ∈ [0, p‑1]
    orient_fns = [
        ("row",   lambda r, c, p: r % p),             # horizontal stripes
        ("minus", lambda r, c, p: (c - r) % p),       # \  diagonals
        ("plus",  lambda r, c, p: (r + c) % p),       # /  diagonals
    ]

    best = None  # (num_assigned, -period, orient_name, pattern_list)

    for name, fn in orient_fns:
        upper = max(h, w)  # the period cannot exceed the longer side
        for p in range(1, upper + 1):
            ...
            for r, c, v in clues:
                ...
                if pat[idx] is None:
                    ...
                    ok = False
                    break  # conflict – this (name, p) pair is invalid
            if not ok:
                continue

            assigned = p - pat.count(None)
            cand = (assigned, -p, name, pat)
            if best is None or cand > best:
                best = cand
            # Early exit: a *perfect* explanation (assigned == p) with the
            # smallest possible period is unbeatable for this orientation.
            if assigned == p:
                break

    # best is guaranteed not None because 'row' with p = len(clues) always works
    _, neg_p, orient_name, pattern = best
    p = -neg_p

    # ------------------------------------------------------------------
    # 3.  Fill any pattern holes with the most common colours in order
    # ------------------------------------------------------------------
    if any(v is None for v in pattern):
        fill_cycle = iter(colours_by_freq)
        for i, v in enumerate(pattern):
            if v is None:
                pattern[i] = next(fill_cycle)

    # Orient‑lookup map for fast dispatch --------------------------------
    def row_idx(r, c):
        return r % p
    ...
    # ------------------------------------------------------------------
    # 4.  Build the finished canvas
    # ------------------------------------------------------------------
    O = [[pattern[idx_fn(r, c)] for c in range(w)] for r in range(h)]
    return O
...
def solve_06df4c85(I):
    ...
    for r, row in enumerate(I):
        # Skip uniform rows (all cells identical)
        if len(set(row)) == 1:
            continue

        # Decide block length k for this row
        k = 3 if (w >= 3 and row[0] == row[2]) else 2

        # Walk blocks safely (avoid over‑run when width % k ≠ 0)
        c = 0
        while c + k - 1 < w:
            a = row[c]                # reference colour
            O[r][c + k - 1] = a       # enforce block symmetry
            c += k
